...
const ViewportToolbar = React.forwardRef<
  HTMLDivElement,
  React.HTMLAttributes<HTMLDivElement>
>(({ className, ...props }, ref) => (
  <ButtonGroup
    ref={ref}
    className={cn('shadow-lg backdrop-blur-xl', className)}
    {...props}
  />
));
ViewportToolbar.displayName = 'ViewportToolbar';

const ViewportToolbarValue = React.forwardRef<
  HTMLDivElement,
  React.HTMLAttributes<HTMLDivElement>
>(({ className, ...props }, ref) => (
  <ButtonGroupText
    ref={ref}
    className={cn('min-w-[3.75rem]', className)}
    {...props}
  />
));
ViewportToolbarValue.displayName = 'ViewportToolbarValue';

interface ViewportToolbarButtonProps
  extends Omit<ButtonProps, 'size' | 'variant'> {
  kind?: ViewportToolbarButtonKind;
}

const viewportToolbarButtonClassNames: Record<
  ViewportToolbarButtonKind,
  string
> = {
  action:
  ...
  icon:
    'px-2.5',
  step:
    'min-w-10 px-0 text-sm font-semibold',
};

const ViewportToolbarButton = React.forwardRef<
  HTMLButtonElement,
  ViewportToolbarButtonProps
