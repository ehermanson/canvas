...
import { fitBoxToViewport, screenToWorld, worldToScreen, zoomPanAtPoint } from "./index";
...
describe("viewport utilities", () => {
  ...
  it("keeps the anchored world point stable when zooming", () => {
    ...
    const worldBefore = screenToWorld({ x: 130, y: 140 }, { pan: { x: 30, y: 40 }, zoom: 2 });
    const worldAfter = screenToWorld({ x: 130, y: 140 }, { pan: next.pan, zoom: next.zoom });
