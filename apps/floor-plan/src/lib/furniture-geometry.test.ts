...
describe('furniture-geometry', () => {
  ...
  it('resizes a rectangle from the dragged edge while keeping the opposite edge fixed', () => {
    expect(
      resizeFurnitureFromEdge(createItem(), 'right', { x: 40, y: 0 }),
    ).toEqual({
      x: 5,
      y: 0,
      width: 70,
    ...
    expect(
      resizeFurnitureFromEdge(createItem(), 'bottom', { x: 0, y: 25 }),
    ).toEqual({
      x: 0,
      y: 5,
      width: 60,
  ...
  it('resizes a rotated rectangle in its local axis', () => {
    const resized = resizeFurnitureFromEdge(
      createItem({ rotation: 90 }),
      'right',
      { x: 0, y: 40 },
    );
  ...
  it('nudges a resize handle by world delta', () => {
    expect(
      resizeFurnitureByHandleDelta(createItem(), 'left', { x: -2, y: 0 }),
    ).toEqual({
      x: -1,
      y: 0,
      width: 62,
    ...
    expect(
      resizeFurnitureByHandleDelta(createItem(), 'top', { x: 0, y: 3 }),
    ).toEqual({
      x: 0,
      y: 1.5,
      width: 60,
  ...
  it('resizes rotated furniture from keyboard handle movement in world space', () => {
    const resized = resizeFurnitureByHandleDelta(
      createItem({ rotation: 90 }),
      'top',
      { x: 1, y: 0 },
    );
  ...
  it('does not flag furniture that stays inside the room polygon', () => {
    expect(
      checkFurnitureRoomCollision(
        createItem({ x: 60, y: 50, rotation: 30 }),
        roomPolygon,
      ),
    ).toBe(false);
  });
  ...
  it('flags rectangular furniture that extends beyond a room wall', () => {
    expect(
      checkFurnitureRoomCollision(createItem({ x: 105, y: 50 }), roomPolygon),
    ).toBe(true);
  });
  ...
  it('snaps nearby furniture edges flush to the closest room wall', () => {
    expect(
      snapFurnitureToRoomWalls(createItem({ x: 89, y: 50 }), roomPolygon, 2),
    ).toMatchObject({
      x: 90,
      y: 50,
    });
  ...
  it('pulls slightly out-of-bounds furniture back inside when close to a wall', () => {
    expect(
      snapFurnitureToRoomWalls(createItem({ x: 91, y: 50 }), roomPolygon, 2),
    ).toMatchObject({
      x: 90,
      y: 50,
    });
  ...
  it('does not snap furniture that is not close to a room wall', () => {
    expect(
      snapFurnitureToRoomWalls(createItem({ x: 75, y: 50 }), roomPolygon, 2),
    ).toMatchObject({
      x: 75,
      y: 50,
    });
