...
function createPulloutState(
  overrides: Partial<PulloutSofaState> = {},
): PulloutSofaState {
  return {
    bedSize: 'queen',
    isOpen: false,
...
describe('pullout-sofa', () => {
  it('returns dimensions for the active open or closed state', () => {
    ...
    expect(
      getPulloutSofaDimensions(createPulloutState({ isOpen: true })),
    ).toEqual({
      width: 72,
      depth: 75,
    });
  ...
  it('normalizes widths so the open state cannot be narrower', () => {
    expect(
      normalizePulloutSofaState(
        createPulloutState({ closedWidth: 72, openWidth: 60 }),
      ),
    ).toMatchObject({
      closedWidth: 72,
      openWidth: 72,
  ...
  it('syncs the item dimensions to the active sofa state', () => {
    ...
    const next = syncPulloutSofaItem(
      item,
      createPulloutState({ isOpen: true, openDepth: 80 }),
    );
  ...
  it('preserves the top edge when opening at 0 degrees', () => {
    ...
    const next = syncPulloutSofaItem(
      item,
      createPulloutState({ isOpen: true, openDepth: 74 }),
      { preserveTop: true },
    );
